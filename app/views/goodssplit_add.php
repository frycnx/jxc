                    ...
                    <div class="panel panel-sky">
                        <div class="panel-heading">
							...
							<div class="options">
								...
								<div class="btn-group">
									<a href='<?php echo url('GoodsSplit')?>' class="btn btn-default"><i class="fa fa-list"></i><span class="hidden-sm"> 拆分列表  </span></a>
								</div>
								</div>
							</div>
                        </div>
                        ...
                        <form method="post" class="form-horizontal" id="goods_form">
                            ...
                            <fieldset>
                            <legend>拆分商品 <div class="btn-group pull-right">
                                    <?php if($row['status']=='0'):?>
                                    <a class="btn btn-sky" data-toggle="modal"  data-target="#one_goods_modal" data-backdrop="static" title="添加商品"><i class="fa fa-plus"></i><span class="hidden-sm"> 添加商品  </span></a>
                                    <?php endif;?>
                                </div></legend>
                            <div class="row form-group">
                            <div class="col-xs-3 text-left">
                                <div class="input-group">
                                  <span class="input-group-addon">SKU</span>
                                   <input type="hidden" name="split_goods_id" id="split_goods_id" value="<?php echo $row['goods_id']?>"/> 
                                  <input type="text" name="split_goods_sku" id="split_goods_sku" class="form-control" readonly="readonly" value="<?php echo $row['goods_sku']?>"/>
                                </div>
							</div>
                            <div class="col-xs-3 text-left">
                                <div class="input-group">
                                  <span class="input-group-addon">商品</span>
                                  <input type="text" name="split_goods_name" id="split_goods_name" class="form-control" readonly="readonly" value="<?php echo $row['goods_name']?>"/>
                                </div>
							</div>
                            <div class="col-xs-3 text-left">
                                <div class="input-group">
                                  <span class="input-group-addon">规格</span>
                                  <input type="text" name="split_goods_spec" id="split_goods_spec" class="form-control" readonly="readonly" value="<?php echo $row['goods_spec']?>"/>
                                </div>
							</div>
                            <div class="col-xs-3 text-left">
                                <div class="input-group">
                                  <span class="input-group-addon">数量</span>
                                  <input type="text" name="split_goods_amount" id="split_goods_amount" class="form-control" value="<?php echo $row['goods_amount']?>"/>
                                </div>
							</div>
                            </div>
                            </fieldset>
                            <fieldset>
                            <legend>拆分为 <div class="btn-group pull-right">
                                    <?php if($row['status']=='0'):?>
...
<?php if($row['status']=='0'):?>
<?php include 'goodssplit_goods_one.php';?>
<?php include 'goodssplit_goods_multi.php';?>
...
function oneGoodsModalFinish(data){
    $("#split_goods_id").val(data['id']);
    $("#split_goods_sku").val(data['sku']);
    $("#split_goods_name").val(data['name']);
    $("#split_goods_spec").val(data['spec']);
    $("#split_goods_amount").select();
    $.post('<?php echo url('GoodsSplit','add')?>',{
        sn:'<?php echo $row['sn']?>',
        goods_id:data['id'],
        shop_id:$("#shop_id").val(),
        memo:$("#memo").val()
...
function save(){
    $.post('<?php echo url('GoodsSplit','save')?>',{
        sn:'<?php echo $row['sn']?>',
        shop_id:$("#shop_id").val(),
        goods_id:$("#split_goods_id").val(),
        goods_amount:$("#split_goods_amount").val(),
        memo:$("#memo").val()
    },function(r){
        showMsg(r['info']);
        if(r['state']) {
...
function addGoods(d, last){
    $.post('<?php echo url('GoodsSplit','addGoods')?>',{
        id:d['id'],
        discount:d['discount'],
        amount:d['amount'],            
        price:d['price'],
...
function loadGoods(){
    $.getJSON('<?php echo url('GoodsSplit','getGoods',array('sn'=>$row['sn']))?>',function(d){
...
function delGoods(id){
    $.get('<?php echo url('GoodsSplit','delGoods')?>&id='+id,function(){loadGoods();});
}
</script>
</body>
</html>