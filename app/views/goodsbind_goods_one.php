    ...
    $("#one_goods_modal_table").ajaxTable({
        ...
        "complete": function(cb){
            $("#one_goods_modal_table tbody tr").dblclick(function(){
                var obj=$(this);
                $("#one_goods_modal").one("hidden.bs.modal",function(){
                    if(typeof oneGoodsModalFinish == "function") {
                        oneGoodsModalFinish({
                                id:obj.children("td:eq(0)").html(),
                                sku:obj.children("td:eq(1)").html(),
                                name:obj.children("td:eq(2)").html(),
                                spec:obj.children("td:eq(3)").html(),
                                barcode:obj.children("td:eq(4)").html(),
                                cate_name:obj.children("td:eq(5)").html(),
                                price_sell:obj.children("td:eq(6)").html(),
                                stock_num:obj.children("td:eq(7)").html(),
                                discount:'1'
                            });
                    }
                }).modal('hide');
