<?php
class TradeReturnGoodsModel extends Model
{
    public function __construct()
    {
        $this->table('order_trade_return_goods');
    }
    ...
    public function changeStock($row)
    {
        ...
        $goods_list = $this->select("SELECT * FROM {$this->table} WHERE trade_return_id={$row['id']}");
        ...
        foreach($goods_list as $goods) {
            if(isset($stock_list[$goods['goods_id']])) {
                $arr_sql[] = "UPDATE {$this->prefix}stock SET stock_num=stock_num+{$goods['amount']},sale_num=sale_num-{$goods['amount']},total_cost=(stock_num+{$goods['amount']})*price_cost WHERE shop_id={$row['shop_id']} AND goods_id={$goods['goods_id']}";
            } else {
