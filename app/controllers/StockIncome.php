...
class StockIncome extends Acl{
    ...
    public function del(){
        ...
        $r = m('StockIncomeGoods')->delete("income_id={$id}");
        if($r === false) {
            $goods_mod->rollBack();
            $this->error($goods_mod->error());
        }
    ...
    public function save(){
        ...
        //处理库存
        $r = m('StockIncomeGoods')->changeStock($row);
        if( !$r ) {
            $mod->rollBack();
            $this->error($mod->error());
        }
    ...
    public function getId($status,$row=array())
    {
        ...
        $data['status'] = $status;
        $data['memo'] = post('memo');
        return m('StockIncome')->add($data,$row);
    }
    ...
    public function getGoods()
    {
        ...
        $data = m('StockIncomeGoods')->getList("income_sn='{$sn}'");
        echo json_encode($data);
    }

    public function addGoods(){
        //增加商品
        $row = m('StockIncome')->getRowBySn(post('sn'));
        $data = array();
        $data['income_id'] = $this->getId('0',$row);
        $data['income_sn'] = post('sn');
        ...
        $r = m('StockIncomeGoods')->add($data);
        if($r === false) {
            $this->error(m('StockIncomeGoods')->error());
        } else {
            $this->success('增加成功！');
        }
    }
    ...
    public function editGoods(){
        ...
        $data['price'] = post('price');
        $data['total'] = (float)$data['discount']*(float)$data['amount']*(float)$data['price'];
        $r = m('StockIncomeGoods')->update($data, "id='{$id}'");
        if($r === false) {
            $this->error(m('StockIncomeGoods')->error());
        } else {
            $this->success('增加成功！');
    ...
    {
        $id = get('id');
        $r = m('StockIncomeGoods')->delete("id={$id}");
        if($r === false) {
            $this->error(m('StockIncomeGoods')->error());
        } else {
            $this->success('删除成功！');
