                    ...
                    <div class="panel panel-sky">
                        <div class="panel-heading">
							...
							<div class="options">
								...
								<div class="btn-group">
									<a href='<?php echo url('Trade')?>' class="btn btn-default"><i class="fa fa-list"></i><span class="hidden-sm"> 销售列表  </span></a>
								</div>
								</div>
							</div>
                        </div>
                        <div class="panel-body">
                            ...
                            <div class="col-xs-4 text-left">
                                <div class="input-group">
                                  <span class="input-group-addon"><input type="checkbox" name="money_check" id="money_check" /></span>
                                  <input type="hidden" name="khyc" id="khyc" value="<?php echo $row['khyc']?>"/>
                                  <input type="text" name="money_memo" id="money_memo" class="form-control" value="从预付金额中扣0.00" disabled="disabled"/>
                                </div>
							</div>
                            </div>
                            <div class="row">
...
<?php if($row['status']=='0'):?>
<?php include 'trade_goods_multi.php';?>
<?php include 'trade_customer.php';?>
<script type="text/javascript">
window.onbeforeunload=function(){if($("#_table tbody tr").length||$("#member_id").val()){return "离开本页则未完成的单据会自动挂账！";}};
$("#yhje").keyup(function(){$("#qkje").val(subtract(subtract(parseFloat($("#ysje").val()||0),parseFloat($("#ssje").val()||0)),parseFloat($("#yhje").val()||0)))});
$("#ssje").keyup(function(){$("#qkje").val(subtract(subtract(parseFloat($("#ysje").val()||0),parseFloat($("#ssje").val()||0)),parseFloat($("#yhje").val()||0)))});
$("#money_check").click(function(){ 
    $("#ssje").val("").select();$("#yhje").val("0.00");$("#qkje").val("0.00");
    var ysje = parseFloat($("#_table tfoot tr:first th:eq(2)").html()||0),myje = parseFloat($("#customer_money").val()||0);
    if($(this).prop("checked")){
        if(ysje>myje){$("#ysje").val(subtract(ysje,myje));$("#khyc").val(myje);}else{$("#ysje").val("0.00");$("#khyc").val(ysje);}
    } else {
        $("#ysje").val(ysje);$("#khyc").val("0.00");
    }
});
...
function addGoods(d, last){
    $.post('<?php echo url('Trade','addGoods')?>',{
        goods_id:d['id'],
        discount:d['discount'],
        amount:d['amount'],            
        price:d['price'],
...
function delGoods(id){
    $.get('<?php echo url('Trade','delGoods')?>',{id:id},function(){loadGoods();});
}
<?php else:?>
<script type="text/javascript">
<?php endif;?>
loadGoods();
function calcMoney(){
    var ysje = parseFloat($("#ysje").val()),myje = parseFloat($("#customer_money").val());
    if(ysje>0 && myje>0) {
        if(ysje > myje) {
            $("#money_memo").val("从预付金额中扣" + myje.toFixed(2));
        } else {
            $("#money_memo").val("从预付金额中扣" + ysje.toFixed(2));
        }
    }
}
function loadGoods(){
    $.getJSON('<?php echo url('Trade','getGoods',array('sn'=>$row['sn']))?>',function(d){
