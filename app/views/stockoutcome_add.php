                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading">
							...
							<div class="options">
								...
								<div class="btn-group">
									<a href='<?php echo url('StockOutcome')?>' class="btn btn-default"><i class="fa fa-list"></i><span class="hidden-sm"> 出库列表  </span></a>
								</div>
								</div>
							</div>
                        </div>
                        <div class="panel-body">
                            ...
                            <div class="col-xs-2 text-left">
                                <div class="input-group">
                                  ...
                                  <option value="">---</option>
                                  <option value="6" <?php echo $row['type']=='5'?'selected="selected"':''?>>采购退货</option>
                                  <option value="6" <?php echo $row['type']=='6'?'selected="selected"':''?>>商品报损</option>
                                  <option value="7" <?php echo $row['type']=='7'?'selected="selected"':''?>>商品借货</option>
                                  <option value="8" <?php echo $row['type']=='8'?'selected="selected"':''?>>其他出库</option>
                                  </select>
                                </div>
							</div>
                            <div class="col-xs-2 text-left">
...
<?php if($row['status']=='0'):?>
<?php include 'stockoutcome_goods_multi.php';?>
...
function save(){
    ...
    $.post('<?php echo url('StockOutcome','save')?>',{
        sn:'<?php echo $row['sn']?>',
        type:$("#type").val(),
        shop_id:$("#shop_id").val(),
        memo:$("#memo").val()
...
function addGoods(d, last){
    ...
    $.post('<?php echo url('StockOutcome','addGoods')?>',{
        goods_id:d['id'],
        discount:d['discount'],
        amount:d['amount'],            
        price:d['price'],
...
function loadGoods(){
    $.getJSON('<?php echo url('StockOutcome','getGoods',array('sn'=>$row['sn']))?>',function(d){
...
function delGoods(id){
    $.get('<?php echo url('StockOutcome','delGoods')?>&id='+id,function(){loadGoods();});
}
</script>
</body>
</html>