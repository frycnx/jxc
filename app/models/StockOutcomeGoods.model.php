<?php
class StockOutcomeGoodsModel extends Model
{
    public function __construct()
    {
        $this->table('stock_outcome_goods');
    }
    ...
    public function changeStock($row)
    {
        ...
        $goods_list = $this->select("SELECT * FROM {$this->table} WHERE outcome_id={$row['id']}");
        ...
        foreach($goods_list as $goods) {
            if(isset($stock_list[$goods['goods_id']])) {
                $arr_sql[] = "UPDATE {$table} SET stock_num=stock_num-{$goods['amount']},recent_sell={$goods['price']},price_cost=(total_cost-{$goods['amount']}*{$goods['price']})/(stock_num-{$goods['amount']}),total_cost=total_cost-{$goods['amount']}*{$goods['price']} WHERE shop_id={$row['shop_id']} AND goods_id={$goods['goods_id']}";
            } else {
                $arr_sql[] = "INSERT INTO {$table}(shop_id,shop_name,goods_id,goods_sku,stock_num,sale_num,price_buy,price_sell,recent_buy,recent_sell,price_cost,total_cost) VALUES( {$row['shop_id']},'{$row['shop_name']}','{$goods['goods_id']}','{$goods['goods_sku']}',-{$goods['amount']},0,{$goods['price']},0,0,{$goods['price']},{$goods['price']},-{$goods['price']}*{$goods['amount']})";
            }
        }
